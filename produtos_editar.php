...
<body>
    ...
    if ($objProduto->getQuantidade() <= 0 || $objProduto->getValor() <= 0) {
        ...
        Falha ao atualizar o produto <span style='font-weight:bold;'>{$objProduto->getNome()}</span>.</br>Quantidade ou valor defenido como zero, <a href='./produtos_listar.php' class='link'>tente novamente</a>.
        </div>
        ";
    } else {
        try {
            mysqli_query($objDatabase->InicioDatabase(), $objDatabase->AtualizarProduto($objProduto->getNome(), $objProduto->getValor(), $objProduto->getQuantidade(), $objProduto->getFabricacao(), $objProduto->getValidade(), $objProduto->getId()));
            echo
            "
            <div class='alert alert-success' role='alert' style='text-align:center;'>
            <i class='bi bi-info-circle'></i>&nbsp;Produto <span style='font-weight:bold;'>{$objProduto->getNome()}</span> atualizado com sucesso, <a href='./produtos_listar.php' class='link'>voltar</a>.
            </div>
            ";
            ...
            "
            <div class='alert alert-danger' role='alert' style='text-align:center;'>
            <span style='font-weight:bold;'><i class='bi bi-exclamation-circle'></i>&nbsp;Fatal Error Exception</span></br>{$exception}</br><a href='./produtos_listar.php' class='link'>Voltar</a>
            </div>
            ";
            mysqli_close($objDatabase->InicioDatabase());
        }
