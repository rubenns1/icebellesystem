...
<body>
    ...
    <table class="table table-hover table-sm" style="text-align:center;">
        ...
        $queryExecute = "SELECT PRODUTOS.ID, PRODUTO, LOTE, PRECO, QUANTIDADE, DATE_FORMAT(DATA_CADASTRO, '%d/%m/%Y') AS CADASTRO, PRECO * QUANTIDADE AS TOTAL, DATE_FORMAT(DATA_FABRICACAO, '%d/%m/%Y') AS FABRICACAO, DATE_FORMAT(DATA_VALIDADE, '%d/%m/%Y') AS VALIDADE FROM PRODUTOS ORDER BY QUANTIDADE > 0 DESC, VALIDADE ASC";
        $executeQuery = mysqli_query($mysqliStart, $queryExecute);

        while ($getValue = mysqli_fetch_assoc($executeQuery)) {
            if ($getValue["ID"] > 0 && $getValue["QUANTIDADE"] > 0) {
                echo
                "
                ...
                <td>{$getValue["LOTE"]}</td>
                <td>{$getValue["PRECO"]}</td>
                <td><span style='font-weight:bold;'>{$getValue["QUANTIDADE"]}</span></td>
                <td><span style='font-weight:bold;'>{$getValue["TOTAL"]}<span></td>
                <td>{$getValue["CADASTRO"]}</td>
                <td>{$getValue["FABRICACAO"]}</td>
                <td class='text-danger'>{$getValue["VALIDADE"]}</td>
                <td>
                <div class='dropdown'>
                ...
                <a class='dropdown-item' href='#'><i class='bi bi-pencil-square'>&nbsp;&nbsp;</i>Modificar</a>
                <form action='./produtos_delete.php' method='post'>
                <button class='btn-primary dropdown-item' type='submit'><i class='bi bi-trash'>&nbsp;&nbsp;</i>Excluir</a>
