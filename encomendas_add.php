...
<body>
    ...
    $cliente1 =  "INSERT INTO ENCOMENDAS(PRODUTO, QUANTIDADE, DATA_ENTREGA, ID_CLIENTE, ID_EMPRESA, TOTAL, STATUS) VALUES('{$getProduto}', $getQuantidade, STR_TO_DATE('{$getDataEntrega}', '%d/%m/%Y'), (SELECT ID FROM CLIENTES WHERE NOME = '{$getCliente}'), (SELECT ID_EMPRESA FROM CLIENTES WHERE CLIENTES.NOME = '{$getCliente}'), (SELECT (PRECO - 1) * $getQuantidade FROM PRODUTOS WHERE PRODUTO = '{$getProduto}'), '-')";
    $insertEncomenda =  "INSERT INTO ENCOMENDAS(PRODUTO, QUANTIDADE, DATA_ENTREGA, ID_CLIENTE, ID_EMPRESA, TOTAL, STATUS) VALUES('{$getProduto}', $getQuantidade, STR_TO_DATE('{$getDataEntrega}', '%d/%m/%Y'), (SELECT ID FROM CLIENTES WHERE NOME = '{$getCliente}'), (SELECT ID_EMPRESA FROM CLIENTES WHERE CLIENTES.NOME = '{$getCliente}'), (SELECT PRECO * {$getQuantidade} FROM PRODUTOS WHERE PRODUTO = '{$getProduto}'), '-')";
    ...
    if ($getQuantidade == 0 || $getQuantidade < 0) {
        ...
        try {
            ...
            if(strpos($getCliente, "Cliente") !== false && strpos($getProduto, "Bolo") !== false)
            {
                ...
                ";
                mysqli_query($mysqliStart, $cliente1);
                ...
                <div class="modal-body">
                    <form style="text-align:center;">
                        ...
                        echo "
                            ...
                            <br>
                            <span style='font-weight:bold;'>Valor Total: R$$setInfo[3]</span>";
                        ?>
                    </form>
                    <br>
