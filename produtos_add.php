...
<body>
    ...
    $getFabricacao = $_POST["FABRICACAO"];
    $getValidade = $_POST["VALIDADE"];
    ...
    $queryMySQL = "INSERT INTO PRODUTOS(PRODUTO, LOTE, PRECO, QUANTIDADE, DATA_CADASTRO, DATA_FABRICACAO, DATA_VALIDADE) VALUES('{$getProduto}', {$setLote}, {$getValor}, {$getQuantidade}, NOW(), STR_TO_DATE('{$getFabricacao}', '%d/%m/%Y'), STR_TO_DATE('{$getValidade}', '%d/%m/%Y')) ON DUPLICATE KEY UPDATE PRECO = {$getValor}, QUANTIDADE = QUANTIDADE + {$getQuantidade}, DATA_FABRICACAO = STR_TO_DATE('{$getFabricacao}', '%d/%m/%Y'), DATA_VALIDADE = STR_TO_DATE('{$getValidade}', '%d/%m/%Y');";

    if ($getQuantidade <= 0 || $getValor <= 0) {
        echo
        "
        <div class='alert alert-danger' role='alert' style='text-align:center;'>
        <span style='font-weight:bold;'><i class='bi bi-exclamation-circle'></i>&nbsp;Fatal Error</span></br>
        Falha ao cadastrar o produto <span style='font-weight:bold;'>{$getProduto}</span>.</br>Quantidade ou valor defenido como zero, <a href='./produtos.php' class='link'>tente novamente</a>.
        </div>
        ";
    } else {
        try {
            mysqli_query($mysqliStart, $queryMySQL);
            echo
            "
            <div class='alert alert-success' role='alert' style='text-align:center;'>
            <i class='bi bi-info-circle'></i>&nbsp;Produto <span style='font-weight:bold;'>{$getProduto}</span> cadastrado com sucesso, <a href='./produtos.php' class='link'>voltar</a>.
            </div>
            ";
