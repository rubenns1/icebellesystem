...
<body>
  ...
  <thead>
    ...
    $queryExecute = "SELECT CLIENTES.ID, CLIENTES.NOME, CLIENTES.WHATSAPP, EMPRESAS.nome AS EMPRESA, DATE_FORMAT(CLIENTES.DATA_CADASTRO, '%d/%m/%Y') AS DATA_CADASTRO FROM CLIENTES JOIN EMPRESAS ON EMPRESAS.ID = CLIENTES.ID_EMPRESA ORDER BY CLIENTES.NOME";
    ...
    while ($getValue = mysqli_fetch_assoc($executeQuery)) {
        if ($getValue["ID"] > 0)
        {
            ...
            <td scope=''>{$getValue["NOME"]}</td>
            <td scope=''>{$getValue["EMPRESA"]}</a></td>
            <td scope=''>+{$getValue["WHATSAPP"]} (<a href='https://wa.me/{$getValue["WHATSAPP"]}' target='_blank'>Mensagem</a>)</td>
            <td scope=''>{$getValue["DATA_CADASTRO"]}</td>
            <td scope=''>
            <div class='dropdown'>
                ...
                <a class='dropdown-item' href='#' ><i class='bi bi-pencil-square text-primary'>&nbsp;&nbsp;</i>Editar</a>
                <form action='./clientes_delete.php' method='post'>
                <button class='btn-primary dropdown-item' type='submit' name='{$getValue["ID"]}'>
