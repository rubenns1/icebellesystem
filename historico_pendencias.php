...
<body>
  ...
  <table class='table table-hover table-sm' style='text-align:center;'>
    ...
    <tbody>
      ...
      $queryExecute = "SELECT HISTPAG.ID, CLIENTES.NOME AS CLIENTE, EMPRESAS.NOME AS UNIDADE, ENCOMENDAS.ID AS ENCOMENDA_ID, ENCOMENDAS.PRODUTO AS PRODUTO, VALOR, DATE_FORMAT(DATA_RECEBER, '%d/%m/%Y') AS RECEBIMENTO, VALOR_RECEBIDO AS RECEBIDO, HISTPAG.DOC_FILE AS DOC_FILE FROM HISTPAG JOIN CLIENTES JOIN EMPRESAS JOIN ENCOMENDAS ON HISTPAG.ID_CLIENTE = CLIENTES.ID AND HISTPAG.ID_EMPRESA = EMPRESAS.ID AND ENCOMENDAS.ID = HISTPAG.ID_ENCOMENDA ORDER BY RECEBIMENTO ASC, RECEBIDO != 0 ASC";
