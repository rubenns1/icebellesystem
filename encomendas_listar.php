...
<body>
  ...
  <table class="table table-hover table-sm" style="text-align:center;">
    ...
    $queryExecute = "SELECT ENCOMENDAS.ID, CLIENTES.NOME AS CLIENTE, EMPRESAS.NOME AS EMPRESA, ENCOMENDAS.QUANTIDADE AS QUANTIDADE, DATE_FORMAT(ENCOMENDAS.DATA_ENTREGA, '%d/%m/%Y') AS ENTREGA, DATE_FORMAT(ENCOMENDAS.DATA_CADASTRO, '%d/%m/%Y') AS CADASTRO, ENCOMENDAS.TOTAL AS TOTAL, ENCOMENDAS.STATUS AS STATUS, ENCOMENDAS.PRODUTO FROM ENCOMENDAS JOIN CLIENTES JOIN EMPRESAS ON ENCOMENDAS.ID_CLIENTE = CLIENTES.ID AND ENCOMENDAS.ID_EMPRESA = EMPRESAS.ID ORDER BY STATUS ASC, ENTREGA ASC, CLIENTE ASC";
    $executeQuery = mysqli_query($mysqliStart, $queryExecute);

    while ($getValue = mysqli_fetch_assoc($executeQuery)) {
      if ($getValue["ID"] > 0 && ($getValue["STATUS"] == "Entregue"))
      {
        echo
        "
                ...
                <td>{$getValue["TOTAL"]}</td>
                <td>{$getValue["CADASTRO"]}</td>
                <td>{$getValue["ENTREGA"]}</td>
                <th><i class='bi bi-check-circle' style='color:green;'></i></th>
                <td>
                <div class='dropdown'>
