...
$listPedencias = "SELECT HISTPAG.ID, CLIENTES.NOME AS CLIENTE, EMPRESAS.NOME AS UNIDADE, VALOR, DATE_FORMAT(DATA_RECEBER, '%d/%m/%Y') AS RECEBIMENTO FROM HISTPAG JOIN CLIENTES JOIN EMPRESAS ON HISTPAG.ID_CLIENTE = CLIENTES.ID AND HISTPAG.ID_EMPRESA = EMPRESAS.ID WHERE HISTPAG.ID = {$newReturnId} AND CLIENTES.NOME = '{$newReturnNome}'";
$executeListPendencias = mysqli_query($mysqliStart, $listPedencias);

while($getPendencias = mysqli_fetch_assoc($executeListPendencias))
{
    $setMessage =  "Valor de *R$".trim($getPendencias["VALOR"], "-")."*%0AData de Pagamento estimado, *{$getPendencias["RECEBIMENTO"]}*";
}
