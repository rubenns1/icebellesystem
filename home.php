...
<body>
      ...
      <ul class="navbar-nav mr-auto">
        ...
        <li class="nav-item dropdown">
          ...
          <div class="dropdown-menu" aria-labelledby="navbarDropdown">
            ...
            <a class="dropdown-item disabled" href="#"><i class="bi bi-cart-check">&nbsp;&nbsp;</i>Fornecedores</a>
  ...
  <?php
  ListarColuna("ID", "PRODUTO", "QUANTIDADE", "PRECO", "FATURAMENTO");

  function ListarColuna($getID, $getNome, $getQuantidade, $getValor, $getFaturamento)
  {
    ...
    $queryMySQL = "SELECT * FROM PRODUTOS";
    $runQuery = mysqli_query($mysqliStart, $queryMySQL);

    while ($mostrarResultados = mysqli_fetch_assoc($runQuery)) {
      if ($mostrarResultados[$getQuantidade] > 0) {
        echo "
        <div class='card text-white bg-info mb-3' style='text-align:center;displpay:inline-block; max-width:auto; margin:0.9em; box-shadow: 1px 1px 20px #808080;'>
            <div class='card-header'><b>Disponível</b></div>
            <div class='card-body'>
            <h5 class='card-title'><b>{$mostrarResultados[$getNome]}</b>
            </h5>
            <class='card-text'>Valor unitário: <b>R$ {$mostrarResultados[$getValor]}</b>
            </div>
            </div>
        ";
      } else if ($mostrarResultados[$getQuantidade] < 1) {
        echo "
            <div class='card text-white bg-danger mb-3' style='text-align:center;displpay:inline-block; max-width:auto; margin:0.9em; box-shadow: 1px 1px 20px #808080;'>
            <div class='card-header'><b>Indisponível</b></div>
            <div class='card-body'>
            <h5 class='card-title'><b>{$mostrarResultados[$getNome]}</b>
            </h5>
            <class='card-text'>Valor unitário: <b>R$ {$mostrarResultados[$getValor]}</b>
